...
interface ConsentementConsultationProps {
  onBackToList: () => void;
  onCreateNew: () => void;
  onSelectPatient: (patientNumber: string) => void;
}

const ConsentementConsultation: React.FC<ConsentementConsultationProps> = ({ 
  onBackToList, 
  onCreateNew, 
  onSelectPatient 
}) => {
  const [forms, setForms] = useState<ConsentementFormData[]>([]);
  ...
  const handleDeleteForm = async (patientNumber: string) => {
    ...
    try {
      ...
      await loadForms();
      alert('Consentement supprimé avec succès');
    } catch (error) {
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-6">
              ...
              />
              <div>
                <h1 className="text-2xl font-bold text-slate-700">Consultation des Formulaires</h1>
                <p className="text-gray-600">Consentement Anesthésique</p>
              </div>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          {filteredForms.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">Aucun consentement trouvé</h3>
              <p className="text-gray-500">
                {searchTerm 
                  ? 'Aucun consentement ne correspond à votre recherche.'
                  : 'Aucun consentement n\'a encore été créé.'
                }
              </p>
            </div>
          )}
...
export default ConsentementConsultation;
